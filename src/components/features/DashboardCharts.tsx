...
export function DashboardCharts({ stats }: DashboardChartsProps) {
  ...
  const data = React.useMemo(() => {
    return (
      stats?.monthlyTrends?.slice(-6).map((m: any) => ({
        ...m,
              ...
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="glass-card rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-6 shadow-2xl border-white/10 backdrop-blur-xl transition-all duration-300 min-w-[180px] sm:min-w-[220px]">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/50 mb-4 sm:mb-5 font-bold italic">
                      {item.month}
                    </p>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs font-bold text-foreground/80 lowercase">
                            Inflow &nbsp;
                          </span>
                        </div>
