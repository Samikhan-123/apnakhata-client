    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
          <div
            key={i}
            className="glass-card rounded-[2rem] p-5 flex items-center gap-5 border-border/20"
          >
    ...
    return (
      <div className="glass-card rounded-[2.5rem] p-24 text-center border-dashed border-2 border-primary/10">
        ...
        <p className="text-muted-foreground font-medium mt-2 max-w-xs mx-auto text-sm">
          Create categories to start organizing your ledger entries
          professionally.
        </p>
      </div>
    );
      ...
      >
        <DialogContent className="sm:max-w-[450px] p-0 border-none bg-transparent shadow-none">
          <div className="glass-card rounded-[2.5rem] p-8">
            ...
            <CategoryForm
              initialData={(editingCategory as any) || undefined}
              onSuccess={() => {
