...
export const LedgerEntryForm = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      description:
        initialData?.description || (isIncomeRequired ? "Salary" : ""),
  ...
  const isExceedingBudget =
    remainingBudget !== null && Number(amountValue) > remainingBudget;
  ...
  const onSubmit = async (values: LedgerEntryInput) => {
    ...
    if (values.type === "EXPENSE" && !initialData) {
      ...
      if (Number(values.amount) > (remainingBalance || 0)) {
        toast.error(
          `Insufficient Balance! Your current balance is ${formatCurrency(remainingBalance || 0)}.`,
        );
        return;
      }
    }
      ...
      {isIncomeRequired && !initialData && (
        <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div className="space-y-1">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-emerald-700">
              Add Income First
            </h4>
            <p className="text-[11px] font-bold text-emerald-600/70 leading-relaxed italic">
              Your current balance is zero. Please record an income (salary,
              business, etc.) to start tracking your expenses!
            </p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
          ...
          >
            ...
            <div>
              <RadioGroupItem
                value="EXPENSE"
                id="expense"
                disabled={isIncomeRequired && !initialData}
                className="peer sr-only"
              />
              <Label
                htmlFor="expense"
        ...
        <div className="grid gap-10">
          ...
          <div className="space-y-4">
            <Label className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground/40 px-1 flex items-center gap-2">
              <Banknote className="h-3.5 w-3.5 text-primary" /> Amount (
              {currency})
            </Label>
          ...
          <div className="space-y-4">
            ...
            <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
              ...
              <PopoverContent
                className="w-auto p-0 rounded-3xl border border-border mt-2 shadow-2xl"
                align="start"
              >
                <Calendar
                  mode="single"
          ...
          <div className="space-y-4">
            ...
            <Input
              placeholder={
                type === "EXPENSE"
                  ? "What was this expense for? (e.g. Food, Shopping)"
                  : "What was this income from? (e.g. Salary, Business)"
              }
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={cn(
                          ...
                          isExceedingBudget ? "bg-rose-500" : "bg-primary",
                        )}
                      />
                  ...
                  <div className="h-1.5 w-full bg-muted/30 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min((Number(activeBudget.spent) / Number(activeBudget.limit)) * 100, 100)}%`,
                      }}
                    />
                  </div>
                  ...
                  {isExceedingBudget && (
                    <p className="mt-2 text-[9px] font-bold italic flex items-center gap-1">
                      <AlertCircle size={10} />
                      Warning: This entry will exceed your{" "}
                      {capitalize(
                        categories.find((c) => c.id === selectedCategoryId)
                          ?.name || "",
                      )}{" "}
        ...
        <Button
          ...
          className={cn(
            ...
            readOnly &&
            "opacity-50 grayscale cursor-not-allowed hover:bg-muted",
          )}
        >
          {readOnly
