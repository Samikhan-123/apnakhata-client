...
import { FileSpreadsheet, FileText, Download, X, Filter, RotateCcw } from 'lucide-react';
...
export function LedgerFilters({ onFilterChange, categories, onExport, currentFilters }: LedgerFiltersProps) {
  ...
  const getActiveChips = () => {
    ...
    if (type !== 'all') chips.push({ key: 'type', label: 'Type', value: capitalize(type) });
    if (categoryId !== 'all') {
      const cat = categories.find(c => c.id === categoryId);
      chips.push({ key: 'categoryId', label: 'Category', value: cat?.name || 'Unknown' });
    }
        ...
        <div className="flex items-center gap-3">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[200px] rounded-2xl p-2 shadow-2xl border-border/40">
              ...
              <DropdownMenuItem onClick={() => onExport('excel')} className="rounded-xl flex items-center gap-3 px-3 py-2.5 cursor-pointer">
                <div className="h-8 w-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-600">
                  <FileSpreadsheet className="h-4 w-4" />
                </div>
                <span className="text-xs font-bold">Excel Records</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onExport('pdf')} className="rounded-xl flex items-center gap-3 px-3 py-2.5 cursor-pointer">
          ...
          >
            <div className="pt-4 mt-4 border-t border-border/10 flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-2 mr-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/30">Active Filters</span>
              </div>
              ...
              {getActiveChips().map((chip) => (
                ...
                >
                  <span className="text-[9px] opacity-40 uppercase font-black">{chip.label}:</span>
