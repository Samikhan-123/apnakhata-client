...
import { Lock, UserPlus, ShieldAlert, AlertCircle, Info } from 'lucide-react';
...
export const AdminStatusBanner = ({ maintenanceMode, registrationEnabled }: UnifiedStatusProps) => {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-[100] pointer-events-none px-6">
      <SlideIn direction="up" distance={20} duration={0.6} className="pointer-events-auto max-w-2xl mx-auto">
        <div className={cn(
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] leading-none mb-1 opacity-70">Security Protocol</span>
              <h4 className="text-xs font-black tracking-tight uppercase tracking-widest leading-none">System Restricted</h4>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 justify-end sm:justify-center">
            {maintenanceMode && (
              <div className="flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/5 whitespace-nowrap">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest">Maintenance Active</span>
              </div>
            )}
            {!registrationEnabled && (
              <div className="flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/5 whitespace-nowrap">
                <AlertCircle size={12} className="text-amber-400" />
                <span className="text-[10px] font-bold uppercase tracking-widest">Signups Paused</span>
              </div>
            )}
          </div>
          
          <div className="hidden lg:flex flex-col items-end opacity-40">
            <span className="text-[9px] font-bold uppercase tracking-[0.1em] leading-none">Admin View</span>
            <span className="text-[8px] font-medium leading-none mt-1 italic">Moderators Enabled</span>
          </div>
        </div>
      </SlideIn>
    </div>
