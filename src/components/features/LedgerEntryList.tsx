    ...
    return (
      <div className="premium-card rounded-3xl p-16 text-center border-dashed">
        ...
        <p className="text-muted-foreground font-medium mt-2 max-w-xs mx-auto text-sm">
          No records found in this sanctuary. Start by logging your first wealth movement.
        </p>
      </div>
    );
  }
      ...
      {groupedEntries.map(([dateLabel, entries]) => (
        <div key={dateLabel} className="space-y-4">
          <div className="flex items-center gap-4 px-2">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/40">{dateLabel}</h3>
            ...
            {entries.map((entry) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={cn(
                    "w-9 h-9 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0 border transition-all duration-500 group-hover:rotate-6",
                    isIncome
                  ...
                  <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="min-w-0 pr-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-0.5 sm:mt-1">
                        <div className="flex items-center gap-1 sm:gap-1.5 text-[9px] sm:text-[11px] font-bold text-muted-foreground/50">
                          ...
                          {isToday(new Date(entry.date)) ? format(new Date(entry.date), 'hh:mm a') : 'Settled'}
                        </div>
                        <span className="hidden sm:inline w-1 h-1 rounded-full bg-border/60" />
                        <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-widest text-primary/50 truncate max-w-20 sm:max-w-none">
                           {entry.category?.name || (isIncome ? 'Income' : 'General')}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="text-left sm:text-right shrink-0">
                      ...
                      <p className="hidden sm:block text-[8px] font-black uppercase tracking-tighter text-muted-foreground/30 mt-1">Movement Confirmed</p>
                    </div>
                  </div>
                    ...
                    {editable ? (
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-11 sm:w-11 rounded-xl hover:bg-muted transition-all active:scale-95 group/btn border border-transparent hover:border-border/10">
                            <MoreVertical className="h-4 w-4 text-muted-foreground/40 group-hover/btn:text-foreground transition-colors" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-48 rounded-2xl p-2 border-border/20 shadow-2xl bg-card/95 backdrop-blur-xl">
                          <DropdownMenuItem 
                            ...
                            disabled={readOnly}
                          >
                            <div className="p-2 bg-primary/10 rounded-lg"><Edit3 className="h-4 w-4" /></div>
                            Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem 
                            ...
                            disabled={readOnly}
                          >
                            <div className="p-2 bg-rose-500/10 rounded-lg"><Trash2 className="h-4 w-4" /></div>
                            Delete
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    ) : (
                      <div className="h-8 w-8 sm:h-11 sm:w-11 flex items-center justify-center opacity-[0.15] cursor-help" title="Secured in sanctuary">
