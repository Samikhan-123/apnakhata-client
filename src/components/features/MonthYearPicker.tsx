...
export const MonthYearPicker = ({
  ...
  const canGoNext = !isFuture(
    selectedMonth === 0 ? 12 : selectedMonth + 1,
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-1 sm:gap-4 premium-card p-1.5 rounded-2xl border-border/40 bg-background/50 backdrop-blur-sm",
        className,
      )}
    >
      <Button
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <div className="flex flex-col items-center min-w-[120px] sm:min-w-[150px] px-2 cursor-pointer group">
            <span
              className={cn(
            ...
            <span className="text-xs sm:text-sm font-black tracking-tight text-foreground group-hover:text-primary transition-colors flex items-center gap-2">
              ...
              <ChevronRight
                size={14}
                className="rotate-90 text-muted-foreground/30"
              />
            </span>
          </div>
        </PopoverTrigger>
        <PopoverContent
          className="w-[300px] p-4 rounded-[2rem] border-border/40 shadow-2xl mt-4"
          align="center"
        >
