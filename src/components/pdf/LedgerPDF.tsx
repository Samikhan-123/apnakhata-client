    ...
    <Document title={`Records Export - ${userName}`}>
      <Page size="A4" style={theme.page}>
          ...
          <View style={theme.metaSection}>
            ...
            {filters?.startDate && (
              <Text style={theme.metaText}>
                Period: {format(new Date(filters.startDate), 'dd MMM')} - {format(new Date(filters.endDate || new Date()), 'dd MMM yyyy')}
              </Text>
            )}
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 20, marginBottom: 20 }}>
          <View>
            <Text style={{ fontSize: 8, color: '#64748b', textTransform: 'uppercase' }}>Total Income</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#059669' }}>
              {(totalIncome || 0).toLocaleString('en-US', { style: 'currency', currency: 'PKR' })}
            </Text>
          </View>
          <View>
            <Text style={{ fontSize: 8, color: '#64748b', textTransform: 'uppercase' }}>Total Expenses</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#dc2626' }}>
              {(totalExpense || 0).toLocaleString('en-US', { style: 'currency', currency: 'PKR' })}
            </Text>
          </View>
          <View>
            <Text style={{ fontSize: 8, color: '#64748b', textTransform: 'uppercase' }}>Net Balance</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#030711' }}>
              {((totalIncome || 0) - (totalExpense || 0)).toLocaleString('en-US', { style: 'currency', currency: 'PKR' })}
            </Text>
          </View>
        </View>
        ...
        <View style={theme.table}>
          ...
          {/* Rows */}
          {(transactions || []).filter(t => t && typeof t === 'object').map((t: any) => (
            <View key={t.id || Math.random()} style={theme.tableRow}>
              <View style={[theme.tableCol, { width: '15%' }]}>
                <Text style={theme.tableCell}>{t.date ? format(new Date(t.date), 'dd MMM yyyy') : '-'}</Text>
              </View>
              <View style={[theme.tableCol, { width: '20%' }]}>
              ...
              <View style={[theme.tableCol, { width: '25%', textAlign: 'right' }]}>
                <Text style={[theme.tableCell, t.type === 'INCOME' ? theme.income : theme.expense]}>
                  {t.type === 'INCOME' ? '+' : '-'}{(t.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </Text>
              </View>
            </View>
          ))}
        ...
        <View style={theme.footer}>
          <Text style={theme.footerText}>Apna Khata - Financial Clarity & Secure Mastery</Text>
