...
export default function LedgerPage() {
  ...
  const fetchData = useCallback(async (currentFilters: any = {}, page: number = 1, silent: boolean = false, forceGlobal: boolean = false) => {
    ...
    setError(null);
    try {
      const [{ data, pagination: pag }, overviewData, globalOverview] = await Promise.all([
        ledgerEntryService.getAll({ ...currentFilters, page, limit: pagination.limit }),
        ledgerEntryService.getOverview(currentFilters),
        (!allTimeOverview || forceGlobal) ? ledgerEntryService.getOverview({}) : Promise.resolve(allTimeOverview)
      ]);
    ...
    <div className=" space-y-10  pb-20">
      ...
      <div className="relative space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { label: 'Available Balance', value: allTimeOverview?.remainingBalance || 0, icon: Wallet, color: 'primary', isGlobal: true },
            { label: 'Total Inflow', value: overview?.totalIncome || 0, icon: ArrowUpRight, color: 'emerald' },
            { label: 'Total Outflow', value: overview?.totalExpense || 0, icon: ArrowDownLeft, color: 'rose' },
          ].map((stat, i) => (
              ...
              <div className="premium-card rounded-2xl p-5 flex flex-col justify-between group h-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="text-right">
                    <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest block">{stat.label}</span>
                    {stat.isGlobal && <span className="text-[8px] font-bold text-primary/40 uppercase tracking-tighter">Global Total</span>}
                  </div>
                </div>
