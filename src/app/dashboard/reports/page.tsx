...
const ReportCharts = dynamic(() => import('@/components/features/ReportCharts'), { ssr: false });
import {
  Activity,
  ArrowUpRight,
  ArrowDownLeft,
...
export default function ReportsPage() {
  ...
  if (error) return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <ErrorState title="Reports Unavailable" message={error} onRetry={() => fetchStats(filters, true)} />
    </div>
  );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const reportData = {
        summary: { totalIncome: income, totalExpense: expense, netSavings: income - expense, savingsRate: income > 0 ? ((income - expense) / income) * 100 : 0 },
        categoryStats: (stats?.categoryBreakdown || []).map((c: any) => ({ name: c.name, total: c.value, percentage: expense > 0 ? (c.value / expense) * 100 : 0 })),
        activeBudgets: [],
        period: filters
      };
    ...
    <div className="space-y-8 pb-20 px-4 md:px-0">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
        ...
        <div className="no-print">
          <Button onClick={handleExportPDF} disabled={isRefreshing} className="rounded-xl h-12 px-8 font-bold shadow-xl gap-2 bg-primary">
        ...
        {[
          ...
          <FadeIn key={metric.label} delay={0.1 * i}>
            <div className="premium-card p-6 rounded-[2rem] border border-border/10 h-36 relative overflow-hidden group flex flex-col justify-between">
              ...
              <div className={cn("p-2.5 rounded-xl border shadow-sm w-fit",
      ...
      <div className="flex items-center gap-3 px-6 py-3 bg-primary/5 rounded-2xl border border-primary/10 w-fit no-print">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/80">
          Analytics Engine Active: <span className="text-foreground">{filters.startDate ? `${format(new Date(filters.startDate), 'MMM dd')} - ${format(new Date(filters.endDate), 'MMM dd')}` : 'Global Period'}</span>
        </p>
      </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between w-full gap-8 relative z-10 font-bold">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-white/60 font-medium text-sm max-w-lg mt-1">{currentTip?.text}</p>
                </div>
              </div>
              <Button onClick={() => setCurrentTipIndex((prev) => (prev + 1) % tips.length)} className="h-10 px-6 rounded-xl bg-white text-slate-950 hover:bg-white/90 font-bold text-sm">
                Next Insight
              </Button>
            </div>
          </div>
